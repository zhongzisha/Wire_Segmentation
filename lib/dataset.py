...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(data.shape)  # 3x512x512
        # print(mask.shape)  # 1x512x512
...
class LineForeignDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # '%s/%s/images/%s.jpg' % (data_root, subset, prefix)
        # '%s/%s/annotations/%s.png' % (data_root, subset, prefix)
        mask = Image.open('%s/%s/annotations_with_foreign/%s.png' % (self.data_root, self.subset, self.prefixes[idx])).convert('L')
        data = Image.open('%s/%s/images/%s.jpg' % (self.data_root, self.subset, self.prefixes[idx]))
