...
def load_data(data_root, subset):
    print('\033[0;33mload data from {} \033[0m'.format(data_root))
    ...
    if subset == 'test2':
        inds = np.random.choice(np.arange(len(prefixes)), size=min(10, len(prefixes)), replace=False)
    else:
    ...
    for ind in inds:
        prefix = prefixes[ind]
        img = np.asarray(readImg('%s/%s/images/%s.jpg' % (data_root, subset, prefix)), dtype=np.uint8)
        gt = np.asarray(readImg('%s/%s/annotations/%s.png' % (data_root, subset, prefix)), dtype=np.uint8)
        if len(gt.shape) == 3:
            gt = gt[:, :, 0]
    ...
    imgs = np.stack(imgs)
    groundTruth = np.stack(groundTruth)
...
def get_data_train(data_root, patch_height, patch_width, N_patches):
    train_imgs_original, train_masks = load_data(data_root, subset='train')
    # save_img(group_images(train_imgs_original[0:20,:,:,:],5),'imgs_train.png')#.show()  #check original train imgs 

    train_imgs = my_PreProc(train_imgs_original)
    # train_masks = train_masks/255.
    del train_imgs_original
    ...
    if N_patches > 0:
        ...
        return patches_imgs_train.astype(np.float32), patches_masks_train.astype(np.float32)
    else:
        return train_imgs.astype(np.float32), train_masks.astype(np.float32)


def get_data(data_root, subset):
    train_imgs_original, train_masks = load_data(data_root, subset=subset)
    # save_img(group_images(train_imgs_original[0:20,:,:,:],5),'imgs_train.png')#.show()  #check original train imgs
...
def get_data_test_overlap(data_root, subset, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original, test_masks = load_data(data_root, subset=subset)
...
def recompone_overlap(preds, img_h, img_w, stride_h, stride_w):
    ...
    full_prob = np.zeros((N_full_imgs, preds.shape[1], img_h, img_w), dtype=np.float32)
    full_sum = np.zeros((N_full_imgs, preds.shape[1], img_h, img_w), dtype=np.float32)
