    ...
    def __init__(self, planes, layers, kernel=3, block=BasicBlock, inplanes=3):
        ...
        for i in range(0, layers):
            self.down_conv_list.append(
                nn.Conv2d(planes * 2 ** i, planes * 2 ** (i + 1), stride=(2, 2), kernel_size=(kernel, kernel),
    ...
    def __init__(self, planes, layers, kernel=3, block=BasicBlock, inplanes=3):
        ...
        for i in range(0, layers):
            self.up_conv_list.append(
                nn.ConvTranspose2d(planes * 2 ** (layers - i), planes * 2 ** max(0, layers - i - 1), kernel_size=(3, 3),
                                   stride=(2, 2), padding=(1, 1), output_padding=(1, 1), bias=True))
            self.up_dense_list.append(block(planes * 2 ** max(0, layers - i - 1), planes * 2 ** max(0, layers - i - 1)))
...
class LadderNet(nn.Module):
    def __init__(self, inplanes=1, num_classes=2, layers=4, filters=10, ):
        ...
        self.final = nn.Conv2d(in_channels=filters, out_channels=num_classes, kernel_size=(1, 1))
