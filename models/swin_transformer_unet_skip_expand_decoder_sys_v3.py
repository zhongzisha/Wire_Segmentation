...
class FinalPatchUp(nn.Module):
    def __init__(self, input_resolution, embed_dim):
        super().__init__()
        self.input_resolution = input_resolution

        self.Up3 = up_conv(ch_in=embed_dim, ch_out=embed_dim//2)
        # self.Up_conv3 = conv_block(ch_in=embed_dim, ch_out=embed_dim//2)

        self.Up2 = up_conv(ch_in=embed_dim//2, ch_out=embed_dim//4)
        # self.Up_conv2 = conv_block(ch_in=embed_dim//2, ch_out=embed_dim//4)
    ...
    def forward(self, x):
        ...
        d4 = x.view(B, H, W, C).permute(0, 3, 1, 2)
        d3 = self.Up3(d4)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # TODO zzs V3改了这里，也就是在V2的基础上，改了FinalPatchUp那里，改成了卷积
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self,
        ...
        if self.final_upsample == "expand_first":
            ...
            #                               dim_scale=4,
            #                               dim=embed_dim)
            self.up = FinalPatchUp(input_resolution=(img_size // patch_size, img_size // patch_size),
                                   embed_dim=embed_dim)
            self.output = nn.Conv2d(in_channels=embed_dim//4, out_channels=self.num_classes,
