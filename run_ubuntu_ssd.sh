...
elif [ $RUN_TYPE == "test_big" ] && [ $HOSTNAME == "slave" ]; then

  echo "testing in big images ..."
  python detect_gd_line.py \
  --network ${NETWORK} \
  --source /media/ubuntu/Data/val_list_ssd.txt \
  --checkpoint ${LINE_REFINE_SEG_TMP_DIR}/${SAVE_DIR}/best_model.pth \
  --save-dir ${LINE_REFINE_SEG_TMP_DIR}/${SAVE_DIR}/big_results_noBoxes/ \
  --img-size 512 --gap 16 --batchsize 2 --device $GPU_ID

  # --box_prediction_dir /media/ubuntu/Data/mmdetection/work_dirs/faster_rcnn_r50_fpn_dc5_2x_coco_lr0.001_newAug3_v2/outputs_val_1024_256_epoch_17
fi